...
class DySample(nn.Module):
    def __init__(self, in_channels, scale=2, style='lp', groups=4, dyscope=False):
        ...
        # Ensure parameters are valid types and values
        self.scale = int(scale) if isinstance(scale, (int, float)) else 2
        self.style = style if isinstance(style, str) and style in ['lp', 'pl'] else 'lp'
        self.groups = int(groups) if isinstance(groups, (int, float)) and groups > 0 else 4

        # Adjust groups to be compatible with in_channels
        if in_channels < self.groups or in_channels % self.groups != 0:
            # Find the largest divisor of in_channels that's <= groups
            for g in range(min(self.groups, in_channels), 0, -1):
                if in_channels % g == 0:
                    self.groups = g
                    break
            else:
        ...
        # Adjust style if 'pl' requirements are not met
        if self.style == 'pl':
            if in_channels < self.scale ** 2 or in_channels % self.scale ** 2 != 0:
                self.style = 'lp'  # Fall back to 'lp' mode

        # 根据 style 调整通道数
        if self.style == 'pl':
            conv_in_channels = in_channels // self.scale ** 2
        ...
        # 偏移量卷积层
        self.offset = nn.Conv2d(conv_in_channels, out_channels, 1)
        normal_init(self.offset, std=0.001)  # 初始化偏移量卷积层

        # 如果启用动态范围调整，则添加范围控制卷积层
        if dyscope:
            self.scope = nn.Conv2d(conv_in_channels, out_channels, 1, bias=False)

...
This module implements CARAFE upsampling as described in:
...
try:
    ...
    CARAFE_AVAILABLE = True
except ImportError:
    CARAFE_AVAILABLE = False
...
class CARAFEPack(nn.Module):
    ...
    def feature_reassemble(self, x, mask):
        if CARAFE_AVAILABLE:
            ...
            # Fallback to simple upsampling if CARAFE extensions not available
...
# For ultralytics integration - simplified interface
class CARAFEUpsampler(nn.Module):
    """CARAFE upsampler for ultralytics integration.
    
    Args:
        scale_factor (int): upsampling scale factor
        up_kernel (int): kernel size for CARAFE
        up_group (int): group size for CARAFE
        compressed_channels (int): compressed channels
    """
    
    def __init__(self, scale_factor=2, up_kernel=5, up_group=1, compressed_channels=64):
        super().__init__()
        self.scale_factor = scale_factor
        self.up_kernel = up_kernel
        self.up_group = up_group
    ...
    def forward(self, x):
        # Lazy initialization based on input channels
        if self.carafe is None:
            c1 = x.shape[1]  # Get input channels from tensor
            self.carafe = CARAFEPack(
                channels=c1,
                scale_factor=self.scale_factor,
                up_kernel=self.up_kernel,
                up_group=self.up_group,
                compressed_channels=min(self.compressed_channels, c1 // 2)  # Avoid overcompression
            ).to(x.device)

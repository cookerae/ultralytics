...
def bbox_iou_wiou_v3(box1, box2, xywh=True, eps=1e-7, momentum=0.999):
    """
    Calculate WIoU v3 loss between two sets of boxes.
    
    WIoU v3 implements a dynamic non-monotonic focusing mechanism based on normalized quality metrics.
    
    Args:
        box1 (torch.Tensor): First set of boxes, shape (N, 4)
        box2 (torch.Tensor): Second set of boxes, shape (N, 4) or (1, 4)
        xywh (bool): Whether boxes are in xywh format. If False, boxes are in xyxy format
        eps (float): Small value to avoid division by zero
        momentum (float): Momentum factor for running mean of IoU
...
class BboxLoss(nn.Module):
    """Criterion class for computing training losses for bounding boxes with WIoU v3."""

    def __init__(self, reg_max: int = 16, use_wiou_v3: bool = True):
        """Initialize the BboxLoss module with regularization maximum and IoU loss settings."""
        super().__init__()
        self.dfl_loss = DFLoss(reg_max) if reg_max > 1 else None
        self.use_wiou_v3 = use_wiou_v3
    ...
    def forward(
        ...
        # Use WIoU v3 or CIoU based on configuration
        if self.use_wiou_v3:
            iou = bbox_iou_wiou_v3(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False)
        else:
...
class RotatedBboxLoss(BboxLoss):
    ...
    def __init__(self, reg_max: int):
        ...
        super().__init__(reg_max, use_wiou_v3=False)  # Keep original probiou for rotated boxes
...
class v8DetectionLoss:
    """Criterion class for computing training losses for YOLOv8 object detection with WIoU v3."""

    def __init__(self, model, tal_topk: int = 10, use_wiou_v3: bool = True):  # model must be de-paralleled
        ...
        self.bbox_loss = BboxLoss(m.reg_max, use_wiou_v3=use_wiou_v3).to(device)
